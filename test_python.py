...
def test_foglamp():
	...
	# assert data['uptime'] >= 1
	# assert data['safeMode'] == False
	# assert data['dataSent'] == 0
	# assert data['hostName'] == 'localhost.localdomain'
	# assert data['dataRead'] == 0
	# assert data['serviceName'] == 'FogLAMP'
	# assert data['health'] == 'green'
	# assert data['dataPurged'] == 0
	# assert data['authenticationOptional'] == True
